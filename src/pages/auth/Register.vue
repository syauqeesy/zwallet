<template>
  <form class="my-5" @submit.prevent="register">
    <div class="input-group mb-5">
      ...
      <input type="text" class="form-control p-2" :class="{'is-invalid': userNameValidity !== 'valid' && userNameValidity !== 'pending'}" v-model.trim="userName" @blur="validateUserName" placeholder="Enter your username">
      <p class="invalid-feedback mt-1" v-if="userNameValidity === 'blank'">Username is required!</p>
      <p class="invalid-feedback mt-1" v-if="userNameValidity === 'tooShort'">Minimum username is 8 characters!</p>
      <p class="invalid-feedback mt-1" v-if="userNameValidity === 'containsEmptySpaces'">Empty spaces is not allowed!</p>
    </div>
    <div class="input-group mb-5">
      <span class="input-group-text p-2">
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
